...
You do **not** reimplement any logic. You sequence `/kairos:implement-story` and `/kairos:close-story` (their command files are the single source of truth) and you own only what they cannot do from a subagent: the upfront ordering, the between-story decisions, and the final push / PR / worktree teardown.
...
1. **`{project_management_dir}/` has uncommitted changes (modified, staged, *or untracked*) → BLOCK.** The run reads each story's `Status` and rewrites story files + the roadmap **inside the worktree**. If the stories you target — or the roadmap — exist only as uncommitted/untracked files in the main checkout, the worktree won't contain them: the run would mis-read `Status`, target the wrong stories, clobber roadmap edits, or fail outright because the story file is absent. **Stop.** Show exactly what is dirty under `{pm}/` and tell the user to `git add` + commit (or stash) it first, then re-run.
   > Common trip: the target stories were just produced by `/kairos:create-story` and never committed — so they live nowhere on `{default_branch}` and cannot appear in the worktree.
...
> ⛔ `{service}`'s Compose (`{compose}`) isn't prefixed for worktree isolation — worktree tests would collide with prod containers. Run `/kairos:setup-worktree-isolation` on `{default_branch}` and commit it, then re-run `/kairos:implement-epic`. (Aborting — no worktree created.)
...
Then link Claude Code memory so context follows into the worktree (same as `/kairos:implement-story` Phase 2d):
...
- `STATUS: BLOCKED` → **stop the entire run.** Do not start the next story. Surface the `BLOCKED_REASON` to the user and ask how to proceed (fix-and-resume / skip this story / abort). The completed stories stay committed in the worktree; the blocked story stays `in_progress`. Re-running `/kairos:implement-epic` later resumes (Phase 1 reuses the worktree, Phase 0 re-derives the still-open list).
...
Next: re-run /kairos:implement-epic for the remaining stories; the worktree is rejoined automatically.
```
