...
> **It is a convention, not a fact, and it is written before any run exists.** Under `worktree_mode: off` — or any run passing `worktree_mode:off` — the epic is committed on the branch already checked out, and this field then names a branch that was never created. It is declarative: nothing reads it to decide where to commit (`implement-epic` resolves the branch itself, in Preflight). Downstream mirrors that *display* it can therefore show a branch that does not exist, so `/kairos:close-story` rewrites it to the branch the story was actually committed on. Do not remove the field and do not leave it blank here — at creation time the epic convention is the best available answer.
