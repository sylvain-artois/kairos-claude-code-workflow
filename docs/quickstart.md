...
- **Run several stories as one unit:** `/kairos:implement-epic {epic-slug}` for a whole epic, or `/kairos:implement-wave {name} STORY-002 STORY-005 …` for an explicit list that crosses epics — one worktree, one branch, one PR either way. Under `worktree_mode: epic_shared` both run **inside** the worktree: `/kairos:worktree epic-{slug}` first, then `cd` into it and start a session there ([concepts.md §3](concepts.md)).
