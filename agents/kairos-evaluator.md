...
3. **Rule on every `JUDGE` line.** Each carries a rubric and a threshold in `GOAL.md`. Gather the evidence yourself — read the code, run the app, drive it as a user would when the assertion is about behavior — and decide PASS or FAIL against the threshold, quoting what you saw. Never PASS a judge assertion from the generator's notes. A verdict from an earlier `EVAL-{n-1}.md` may be **carried over only when nothing it depends on has moved**: `find {the service paths the row's behavior lives in} -type f -newer {GOAL_DIR}/EVAL-{n-1}.md` lists no such file. Then write `PASS (carried from EVAL-{n-1}: no file under … changed since)` with that command's output as the evidence; any listed file, and you judge again.
   An **owner ruling** in `STATE.md` (a section `### Owner ruling …`, and the orchestrator's prompt naming it) fixes how a row reads when its wording admits two readings. Judge that row against the ruling, say so in the evidence, and never extend a ruling to a row it does not name.
